...
export default function DashboardPage() {
  ...
  const handleToggleVeil = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch(`${apiUrl}/api/connect`, { method: 'POST' });
  ...
  const wardenCoords = useMemo(() => {
    if (!veilIsActive || !connection?.warden_peer_id) return null;
    return peerIdToCoords(connection.warden_peer_id);
              ...
              <Card className="border-border/50 bg-card p-8">
                ...
                <div className="relative aspect-[2/1] overflow-hidden rounded-lg border border-border/50 bg-secondary/30">
                  ...
                  <div className="absolute inset-0">
                    ...
                    {veilIsActive && wardenCoords && (
                      <svg className="absolute inset-0 h-full w-full" xmlns="http://www.w3.org/2000/svg">
                        <line x1="25%" y1="55%" x2={wardenCoords.x} y2={wardenCoords.y} stroke="#FFA500" strokeWidth="2" strokeDasharray="5,5" className="animate-pulse" />
                      </svg>
                    )}
                    {veilIsActive && wardenCoords && (
                      <div className="absolute h-3 w-3 rounded-full bg-accent shadow-[0_0_15px_rgba(255,165,0,0.6)]" style={{ left: wardenCoords.x, top: wardenCoords.y }} />
                    )}
                  </div>
                </div>
              </Card>
